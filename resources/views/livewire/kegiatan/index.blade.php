                    ...
                    <tbody>
                        @foreach($kegiatans as $kegiatan)
                            <tr>
                                ...
                                <td>{{ $kegiatan->nama }}</td>
                                <td>{{ $kegiatan->tgl_pelaksanaan->format('d/m/Y') }}</td>
                                <td>{{ $kegiatan->organisasi->nama_organisasi }}</td>
                                <td>{{ $kegiatan->lokasi->nama_lokasi }}</td>
                                <td style="align-content: center; text-align:center">{{ $kegiatan->kepanitiaans->count() }}</td>
                                <td class="text-center">
                                <a href="{{ route('kegiatan.show', $kegiatan->id) }}" class="btn btn-sm btn-info mb-1" title="Lihat">
                                    <i class="fas fa-eye"></i>
                                </a>
                                
                                <a href="{{ route('kegiatan.edit', $kegiatan->id) }}" class="btn btn-sm btn-warning mb-1" title="Edit">
                                    <i class="fas fa-edit"></i>
                                </a>
                                
                                <button wire:click="delete({{ $kegiatan->id }})" class="btn btn-sm btn-danger mb-1" title="Hapus" onclick="return confirm('Yakin ingin menghapus?')">
                                    <i class="fas fa-trash-alt"></i>
                                </button>

                                <a href="{{ route('kepanitiaan.manage', $kegiatan->id) }}" class="btn btn-sm btn-info">Kelola Panitia</a>
                                </td>
                            </td>
